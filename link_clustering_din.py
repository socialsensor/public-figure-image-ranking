...
by: Konstantinos Konstantinidis
    *modified to accept an adjacency list as a zip using and exports a
    community list with nodes as its entries
usage
partion = ahnsmethod(adjList)
'''
...
def read_edgelist_weighted(adjList):
    ...
    for line in adjList:
        L = line#.strip().split(',')
        ni,nj,wij = str(L[0]),str(L[1]),float(L[2]) # other columns ignored
...
def write_edge2cid(e2c):#,filename,delimiter="\t"):
    ...
    nodeDict = {}
    for idx,cid in enumerate(sorted(cid2edges.keys())):
        # strcid = str(c2c[cid])
        nodes  = cid2nodes[cid]
        nodeDict[idx] = list(nodes)
    return nodeDict
...
Output:
  ...
  third column.
"""
def ahnsmethod(adjList, threshold=None):

    adj,edges,ij2wij = read_edgelist_weighted(adjList)
    ...
    nodeDict = write_edge2cid( edge2cid )
